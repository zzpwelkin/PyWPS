...
class PywpsUnittest(unittest.TestCase):
    """ The base unit test class of pywps """
    inputs = None
    xmldom = None
...
class ParseRequestTest(PywpsUnittest):
...
class LocalFileProcessTest(PywpsUnittest):
    ...
    def testServiceLocalComplexDataFileInput(self):
        """ Input data from service local directory
        
        Note: this feature is not the content of wps standard, just for build a 
        galaxy with galaxy software
        """ 
        self._setFromEnv()
        wps=pywps.Pywps(pywps.METHOD_GET)
        input = wps.parseRequest("version=1.0.0&service=wps&request=execute&identifier=complexRaster&DataInputs=[indata=file://%s]&responsedocument=outdata=]" 
                                   % os.path.abspath(os.path.join(pywpsPath, "tests", "datainputs", "dem.tiff")))
        wps.performRequest()
        
        xmldom = minidom.parseString(wps.response)
        self.assertFalse(len(xmldom.getElementsByTagNameNS(self.wpsns,"ExceptionReport")), 0)
        
    def testServiceLocalComplexDataFileInputWithReferenceOut(self):
        """ Input data from service local directory
        ...
        self._setFromEnv()
        wps=pywps.Pywps(pywps.METHOD_GET)
        input = wps.parseRequest("version=1.0.0&service=wps&request=execute&identifier=complexRasterMapscript&DataInputs=[indata=file://%s]&responsedocument=outdata=@asReference=True]" 
                                   % os.path.abspath(os.path.join(pywpsPath, "tests", "datainputs", "dem.tiff")))
        wps.performRequest()
...
class ExectueInputAsReferenceTest(PywpsUnittest):
...
class ExecuteOuputAsreferenceTest(PywpsUnittest):
...
class OtherTest(PywpsUnittest):
    def test28MetadataOutputDescribeProcessAndExecute(self):
        ...
        if wpsTree.xpath("//wps:ExceptionReport",namespaces=wpsTree.nsmap):
            self.fail(wpsTree.xpath("//ows:ExceptionText",namespaces=wpsTree.nsmap)[0].text )
