...
class WPSProcess:
    ...
    def __init__(self, identifier, jobcls, title=None, abstract=None,
    ...
    def getCmd(self):
        """
        get the cmd of this process from the inputs value and outpus setting
        
        if the process have one output only, then the ouput can not exit in command,as  
            cmd input1 input2
        else the ouputs that want to export must be in command, as
            cmd input1={} input2={} output1={} output2={} ... or cmd input1 input2 ... output1 ouput2 ... 
            or mixed with key-value and list format
            
        if some inputs need more than one value, then the comma need be used as speration.
        
        @return: the command in list
        """
        def getValue(identifier):
            """
            @return: value with string
            """
            v = self.getInput(identifier)
            if v:
    ...
    def OutputReset(self):
            """
            extract the value from output file
            """
            from contextlib import closing
            for identifier, output in self.outputs.iteritems():
                if isinstance(output, InAndOutputs.LiteralOutput):
                    if output.value and os.path.isfile(output.value):
                        fname = output.value
                        with closing(open(output.value, 'r')) as f:
                            output.value = f.read()
                        os.remove(fname)
...
#            if v:
#                self.outputs[identifier].value = tempfile.mktemp(prefix="pywpsOutput",dir=curdir)
#                return (self.outputs[identifier].value, False)
#            
#            raise Exceptions.InvalidParameterValue('The process of {0} not found'.format(identifier))
#        
#        
...
#                    
#            
#        # create the output file for execute
#        cmd.append(self.execFile)
#        isStdout = True
#        
#        cmdllist = self.cmd.strip().split()
#        for param in cmdllist[1:]:
#            _kv = param.split('=')
#            value = getValue(_kv[0])
#            
#            if not value[1]:
...
#            else:
#                cmd.append('='.join([_kv[0], str(value[0])]))
#      
#        # TODO: execute 
#        if isStdout: 
#            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=None)
#        else:
#            p = subprocess.Popen(cmd, env=None)
#            
#        (stdout, stderr) = p.communicate()
#        
#        # TODO: if stdout is not null and set it to output
#        if not isStdout:
#            OutputReset()
#        elif stdout:
#            self.outputs[self.outputs.keys()[0]] = stdout
#        elif stderr:
#            raise Exceptions.ServerError(stderr)
#        
#        
#        
