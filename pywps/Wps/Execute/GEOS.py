...
class GeoServer(MapServer):
    def __init__(self, process, sessId):
        ...
        self.cat = Catalog( '/'.join( (config.getConfigValue('geoserver', 'geoserveraddress'), 'rest') ),
                            config.getConfigValue('geoserver', 'username'),
                            config.getConfigValue('geoserver', 'password') )
        ...
        workspace_name = config.getConfigValue('geoserver', 'workspace')
        
        if workspace_name:
            self.workspace = self.cat.get_workspace(workspace_name)
        else:
            logging.warn("The geoserver.workspace doesn't setted and the default workspace was getted")
            self.workspace = self.cat.get_default_workspace()
            
        if not self.workspace:
            logging.warn("The workspace {0} doesn't exit and the default workspace was getted".format(workspace_name))
            self.workspace = self.cat.get_default_workspace()
